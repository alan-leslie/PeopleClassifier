...
import com.alag.ci.textanalysis.lucene.impl.EqualInverseDocFreqEstimator;
...
public final class LuceneTextAnalyzer implements TextAnalyzer {
    ...
    public List<Tag> analyzeText(String text) throws IOException {
        ...
        int indexOfOpenBracket = theText.indexOf("(");
        while(indexOfOpenBracket >= 0){
            int indexOfCloseBracket = theText.indexOf(")", indexOfOpenBracket); 
            int indexOfHyphen = theText.indexOf("-", indexOfOpenBracket);
            
            if(indexOfCloseBracket > 0 &&
                    indexOfHyphen >= 0 && indexOfHyphen < indexOfCloseBracket){
                int indexOfHyphen2 = theText.indexOf("-", indexOfHyphen + 1);
            ...
            indexOfOpenBracket = theText.indexOf("(", indexOfOpenBracket + 1);
        }
        ...
        while (tokenStream.incrementToken()) {
            ...
            indexOfOpenBracket = theTerm.indexOf("(");
            int indexOfCloseBracket = theTerm.indexOf(")");

            if (indexOfOpenBracket == 0) {
                if (indexOfCloseBracket > 0) {
                    if (indexOfCloseBracket == theTerm.length() - 1) {
                        theTerm = theTerm.substring(0, theTerm.length() - 1);
                    }
                }
                                
                theTerm = theTerm.substring(1);
            } else {
                if (indexOfOpenBracket > 0) {
                    String padding = "";
                    if (indexOfCloseBracket > indexOfOpenBracket + 2) {
                        padding = " ";
                    }
                    String thePrefix = theTerm.substring(0, indexOfOpenBracket);
                    String theSuffix = theTerm.substring(indexOfOpenBracket + 1, theTerm.length());
                    theTerm = thePrefix + padding + theSuffix;
                } else {
                    int termLength = theTerm.length();
                    if (indexOfCloseBracket > 0 && 
                            indexOfCloseBracket == termLength - 1){
                        theTerm = theTerm.substring(0, termLength - 1);  
                    }
                }
            }
